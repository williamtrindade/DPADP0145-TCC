\section{Model-View-Controller (MVC)}
    ...
    \par Esta seção tem como objetivo explorar o MVC desde o seu surgimento na Xerox PARC até as versões mais modernas utilizadas do desenvolvimento web. Essa abordagem ajudará a entender por que mesmo depois de quatro décadas o MVC ainda vem sendo relevante para o desenvolvimento de software, o que corrobora para entender-se como implementar os princípios da arquitetura limpa em um \textit{framework} que usa-o como base.
    
    \par O padrão arquitetural MVC foi desenvolvido em 1978 por Trygve Reenskaug enquanto ele trabalhava na Xerox PARC, que na época era um importante centro de pesquisa e desenvolvimento da Xerox Corporation. O MVC teve inicialmente o objetivo de criar sistemas de informação que refletissem os modelos mentais dos usuários, promovendo uma interação intuitiva por meio de inspeção e edição de dados \cite[p.~1]{artigo:reenskaug:2003}.
        
    \par Ainda nesse contexto, a sua implementação foi realizada inicialmente no ambiente Smalltalk, e foi formalizada como um padrão por Reenskaug em duas notas técnicas publicadas por ele em maio e dezembro de 1979, essa notas estabeleceram as bases do MVC \cite{artigo:reenskaug:1979}.
    ...
    \par Com a evolução tecnológica, o MVC foi adaptado para interfaces mais complexas e posteriormente inspirou \textit{frameworks} modernos, como o ASP.NET MVC, que foi lançado no ano de 2009. Nesse sentido é notável  que o Smalltalk foi o responsável pela criação do padrão MVC, que posteriormente se tornou amplamente adotado em sistemas orientados a objetos \cite{artigo:deacon:2009}.
    

    \par O MVC é um padrão arquitetural que separa o software em três camadas, a camada que contém lógica do domínio da aplicação chamada Model, a camada que é responsável pela apresentação dos dados chamada View e a camada responsável por controlar as interações do usuário com a aplicação chamada Controller, assim promovendo modularidade, reutilização e manutenção do sistema\cite{artigo:reenskaug:2003}.

    \par Nesse contexto de definição de conceitos, nota-se que o coração do padrão MVC não é a separação técnica entre Model, View e Controller, mas a garantia de que o Model espelhe o modelo mental do usuário. Isso significa que, ao interagir com o sistema, o usuário não está lidando com tabelas de Banco de Dados (BD) ou Notação de Objeto JavaScript (JavaScript Object Notation - JSON), mas com representações digitais de conceitos que já fazem parte do seu entendimento (ex.: ``projetos'', ``tarefas'', ``prazos''). Essa abordagem, inspirada em psicologia cognitiva\footnote{A Psicologia Cognitiva é o estudo científico dos processos internos utilizados para extrair sentido do ambiente e decidir a ação apropriada, incluindo atenção, percepção, aprendizagem, memória, linguagem, resolução de problemas, raciocínio e pensamento \cite{livro:eysenck:2017}.} e em linguagens de modelagem como UML, foi o que diferenciou o MVC de padrões anteriores \cite{artigo:reenskaug:2003}.
    ...
    \begin{figure}[H]
        ...
        \fonte{\cite{artigo:reenskaug:2003}.}
    \end{figure}

    %% MODEL 
    
    \par A camada Model, como apresentada na Figura \ref{fig:figura_mvc_1}, representa o núcleo do domínio da aplicação, encapsulando tanto os dados quanto as regras de negócio que regem o comportamento do sistema. Ela deve manter-se completamente independente das questões relacionadas à UI, não contendo qualquer referência direta ou conhecimento sobre como os dados são apresentados ou manipulados externamente \cite{artigo:deacon:2009}.
    ...
    %% Diferenciais
    \par A arquitetura MVC oferece diferenciais significativos para o desenvolvimento e manutenção de sistemas de software complexos. Ao isolar a lógica de negócio das preocupações com UI, torna-se possível modificar ou expandir as camadas de apresentação sem afetar o núcleo do sistema, assim como reutilizar o mesmo Model em diferentes contextos e plataformas \cite{artigo:deacon:2009}.
    ...
    \subsection{Padrão MVC no Smalltalk-80}
        ...
        \par Conforme já mostrado no presente trabalho o MVC tem sua origem no Smalltalk-80, tendo isso em vista pode-se detalhar como funcionava a implementação inicial desse padrão trazendo bons paralelos para as implementações mais modernas desse padrão.
    ...
    \subsection{Padrão MVC-WEB}
        ...
        \par O padrão MVC-Web surge como uma adaptação do tradicional MVC para o contexto de aplicações web, onde a arquitetura cliente-servidor e o Protocolo de Transferência de Hipertexto (Hypertext Transfer Protocol - HTTP) introduzem desafios distintos em relação aos sistemas desktop originais \cite{inproceedings:grove:2011}.
        
        \par Enquanto o MVC clássico foi concebido para ambientes interativos como o Smalltalk-80, o MVC-Web reflete as mudanças necessárias para lidar com a natureza \textit{stateless} (sem estado de sessão) da web, a separação física entre cliente e servidor e a necessidade de gerenciamento explícito do fluxo de navegação \cite{inproceedings:grove:2011}.
        
        %% MODEL
        
        \par No MVC-Web, o Model mantém sua função central de gerenciar o estado da aplicação e suas regras de negócio mas com adaptações significativas \cite{inproceedings:grove:2011}:
    
        \begin{itemize}
            \item Persistência de Dados: Diferentemente do Smalltalk, onde o modelo era persistente na memória, em \textit{frameworks} web como Rails ou ASP.NET, o modelo frequentemente interage com o BD, usando padrões como Active Record ou Mapeamento Objeto-Relacional (Object-Relational Mapping - ORM).
            
            \item Transações e Lógica de Negócio: O modelo continua responsável por executar operações críticas, mas sua comunicação com as views não ocorre mais via Observer (como no padrão original). Em vez disso, o controller atua como intermediário.
            
            \item Interfaces Externas: Em aplicações modernas, o modelo também pode integrar interfaces de programação de aplicações externas ou serviços web, expandindo sua função além do armazenamento local.
        \end{itemize}
    
    
        %% VIEW
    
        \par A View no MVC-Web assume um papel híbrido, combinando renderização \textit{server-side} com interatividade \textit{client-side} \cite{inproceedings:grove:2011}: 
    
        \begin{itemize}
            \item Templates Dinâmicos: \textit{Frameworks} como Rails e ASP.NET usam templates HTML com código embutido para gerar páginas dinamicamente.
    
            \item Interações no lado do cliente: Com tecnologias como JavaScript e Ajax, a view pode realizar validações, atualizações parciais e outras operações sem recarregar a página, assumindo funções que, no MVC original, eram do modelo ou controller.
    
            \item Separação Menos Rígida: Enquanto no Smalltalk a view era passiva (só exibia dados), no MVC-Web ela pode conter lógica de apresentação e até mesmo regras de negócio simples (como validação de formulários), o que desafia a pureza do padrão.
        \end{itemize}
        
       
        %% Controller
        
        \par O Controller no MVC-Web sofreu as mudanças mais radicais, principalmente devido ao Front Controller, um padrão arquitetônico que centraliza o tratamento de requisições HTTP \cite{inproceedings:grove:2011}:
    
        \begin{itemize}
            \item Front Controller: \textit{Frameworks} como ASP.NET MVC, Rails e Struts implementam esse padrão, onde um único componente (como o Router no Rails) centraliza o roteamento de requisições HTTP para controllers específicos.
    
            \item Ações e Fluxo de Navegação: Cada método de um controller (ex: UsersController@create) corresponde a uma ação (como criar um usuário). O controller decide qual view exibir com base no resultado (ex: redirecionar após um sucesso ou mostrar um erro).
    
            \item Validação e Preparação de Dados: O controller filtra e valida parâmetros da requisição (como dados de formulários), delegando apenas a lógica de negócio ao modelo.            
        \end{itemize}