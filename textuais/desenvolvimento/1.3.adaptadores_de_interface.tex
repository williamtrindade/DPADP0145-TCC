\subsection{Camada de Adaptadores de Interface}
    ...
    \par O método \texttt{\_\_invoke()} implementa o padrão Single Action Controller do Laravel, extraindo os dados da requisição HTTP (\texttt{name}, \texttt{phone\_number} e \texttt{email}), criando uma instância de \texttt{CreateContactRequestModel} (um DTO que encapsula os dados necessários), invocando o método \texttt{create()} do interactor para delegar a lógica de negócio, recuperando o modelo de visualização formatado pelo presenter através do método \texttt{getViewModel()}, e retornando uma resposta JSON com código de status HTTP 201 (Created).
    ...
    \par Nessa abordagem da Arquitetura Limpa, os presenters são registrados como singletons pois precisam manter o estado do ViewModel entre a execução do interactor e a recuperação no controller, essa etapa de configuração será vista mais a frente no trabalho.
    ...
    \par A próoxima etapa foi a implementação dos \texttt{Presenters}, eles atuam como adaptadores de saída (output adapters) responsáveis por transformar os dados retornados pelos casos de uso em um formato adequado para a camada mais externa, implementando o padrão de separação entre lógica de negócio e formatação de resposta. 
    ...
    \par Sendo um Singleton, primeiro o interactor invoca \texttt{present()} para formatar os dados, e em seguida o controller invoca \texttt{getViewModel()} para recuperar o resultado formatado, asssim é garantindo que ambas as operações acessem a mesma instância do presenter e, consequentemente, o mesmo \texttt{viewModel} armazenado.
