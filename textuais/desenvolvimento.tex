    ...
    \section {API REST base}
        ...
        \subsection{Instalação do Laravel}
            ...
            \begin{itemize}
                ...
                \item \textbf{Docker Compose} - para orquestrar múltiplos containers com um único arquivo de configuração.
            \end{itemize}
        ...
        \subsection{Docker}
            ...
            \par Para garantir um ambiente de desenvolvimento isolado, consistente e portátil para replicação do projeto, foi utilizada a tecnologia Docker. O uso de containers Docker permitiu encapsular a aplicação e suas dependências, como o servidor web e o banco de dados, eliminando problemas de compatibilidade entre diferentes sistemas operacionais, a orquestração dos serviços foi realizada com o Docker Compose.
            
            \par Conforme a Figura \ref{fig:figura_tutorial_1}, a estrutura de diretórios do projeto foi organizada para acomodar os arquivos de configuração do Docker junto ao projeto Laravel na raiz do mesmo. No diretório reservado ao docker \texttt{/docker}, comtém a pasta \texttt{app} que armazena o Dockerfile do projeto e a pasta \texttt{nginx} que armazena a configuração do servidor web Nginx, que atua como ponto de entrada para as requisições HTTP.
    
            \par Ainda na Figura \ref{fig:figura_tutorial_1} pode-se notar o arquivo \texttt{docker-compose.yml} que é o componente central para a orquestração dos serviços, definindo o uso de containers para a aplicação, servidor web e banco de dados.
            ...
            \par O Dockerfile é o primeiro componente criado para o ambiente Docker, atuando como um 'manual de instruções' para a construção da imagem do container da aplicação. Ele utiliza a imagem base \texttt{php:8.2-fpm-alpine}, garantindo um ambiente leve e focado no processamento de scripts PHP. 
            
            \par Através de comandos RUN, o arquivo instala dependências essenciais como o Git e bibliotecas de desenvolvimento, além de extensões PHP específicas, como a \texttt{pdo\_mysql}, que é necessária para a comunicação com o banco de dados. Por fim, o Dockerfile instala o Composer para gerenciar as dependências do Laravel e define o diretório de trabalho, garantindo que o contêiner esteja pronto para executar a aplicação. A Figura \ref{fig:figura_tutorial_2}, apresenta o código escrito no Dockerfile.
    
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\textwidth]{figuras/tutorial/2docker.png}
                \caption{Código do Dockerfile}
                \label{fig:figura_tutorial_2}
                \newcommand{\minhafonteum}{Fonte: Autor}
                \minhafonteum
            \end{figure}
    
            \par O arquivo \texttt{default.conf}, localizado na pasta \texttt{docker/nginx}, é o responsável por configurar o servidor web Nginx. Sua principal função é atuar como um proxy reverso, recebendo todas as requisições HTTP e encaminhando-as para o container da aplicação PHP.
    
            \par Dentro deste arquivo, a diretiva \texttt{root} é configurada para apontar para a pasta public do Laravel, que é o ponto de entrada da aplicação. A regra de \texttt{location /} garante que todas as requisições para URLs que não correspondem a arquivos existentes sejam redirecionadas para o \texttt{index.php} do Laravel, o que é fundamental para o sistema de roteamento do framework.
    
            \par A seção \texttt{location \textasciitilde \textbackslash.php\$} é a mais crítica para o funcionamento da aplicação. Ela intercepta todas as requisições que terminam em .php e as encaminha para o serviço app (o container PHP-FPM) na porta 9000. Essa configuração permite que o Nginx entregue arquivos estáticos diretamente e passe apenas as requisições dinâmicas para o PHP processar, otimizando o desempenho.
    
            \par Na Figura \ref{fig:figura_tutorial_3} é apresentado o código escrito no arquivo \texttt{default.conf}.
    
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\textwidth]{figuras/tutorial/3docker.png}
                \caption{Código do default.conf}
                \label{fig:figura_tutorial_3}
                \newcommand{\minhafonteum}{Fonte: Autor}
                \minhafonteum
            \end{figure}
            ...
            \begin{enumerate}
                \item \textbf{services}: Esta seção define os containers que compõem a aplicação.
                \begin{itemize}
                    \item \textbf{\texttt{app}}: Este serviço constrói o container da aplicação, utilizando o 
                    
                    \texttt{Dockerfile}. A diretiva \texttt{volumes} serve para que o código do projeto na máquina host seja sincronizado com o código dentro do container.
                    
                    \item \textbf{\texttt{nginx}}: O container do Nginx atua como um servidor web e \textit{proxy reverso}. Ele mapeia a porta \texttt{8000} da máquina host para a porta \texttt{80} do container, permitindo que o acesso à aplicação pelo navegador.
                    
                    \item \textbf{\texttt{db}}: Este serviço cria o contêiner do MySQL. A diretiva \texttt{environment} configura o banco de dados com variáveis definidas no arquivo \texttt{.env}, enquanto a diretiva \texttt{volumes} garante a persistência dos dados. O mapeamento da porta \texttt{3307:3306} permite que o container do banco de dados seja acessado na porta \texttt{3307} da máquina host, evitando conflitos com outras instalações do MySQL.
                \end{itemize}
            ...
            \par Na Figura \ref{fig:figura_tutorial_4} é apresentado o código do arquivo \texttt{docker-compose.yml}.
    
            \begin{figure}[H]
                \centering
                \includegraphics[width=1\textwidth]{figuras/tutorial/4docker.png}
                \caption{Código do docker-compose.yml}
                \label{fig:figura_tutorial_4}
                \newcommand{\minhafonteum}{Fonte: Autor}
                \minhafonteum
            \end{figure}
            ...
            \par É no arquivo .env que as variáveis como \texttt{DB\_CONNECTION}, \texttt{DB\_HOST}, \texttt{DB\_PORT}, \texttt{DB\_DATABASE}, \texttt{DB\_USERNAME}, \texttt{DB\_PASSWORD} e \texttt{DB\_ROOT\_PASSWORD} são ajustadas para corresponderem às configurações do serviço de banco de dados no arquivo docker-compose.yml. Isso permite que o container da aplicação Laravel se conecte corretamente ao container do banco de dados, estabelecendo a comunicação necessária para o funcionamento do projeto.

            \par A Figura \ref{fig:figura_tutorial_5} mostra como ficaram as configurações alteradas no arquivo \texttt{.env.example}:

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.8\textwidth]{figuras/tutorial/5docker.png}
                \caption{Código do .env.example}
                \label{fig:figura_tutorial_5}
                \newcommand{\minhafonteum}{Fonte: Autor}
                \minhafonteum
            \end{figure}

            \par Após essa alteração foi rodado o seguinte comando no terminal para a criação do arquivo \texttt{.env}:
            ...
            \par A flag \texttt{-d} (modo detached) executa esses containers em segundo plano, liberando o terminal para outras atividades. Por fim, a flag \texttt{--build} garante que as imagens dos containers sejam reconstruídas a partir dos arquivos Dockerfile antes de serem iniciadas, assegurando que o ambiente esteja sempre atualizado com as últimas modificações no código-fonte ou nas dependências.
            ...
            \par A última etapa da configuração do ambiente foi a execução do seguinte comando, para rodar as migrações do Laravel dentro do container da aplicação:
            ...
            \sloppy
            \par Após essas etapas o ambiente docker foi verificado com o comando \texttt{docker-compose ps} e demonstrou sucesso na execução dos containers conforme a Figura \ref{fig:figura_tutorial_6}.

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.9\textwidth]{figuras/tutorial/6docker.png}
                \caption{Resultado do comando docker-compose ps}
                \label{fig:figura_tutorial_6}
                \newcommand{\minhafonteum}{Fonte: Autor}
                \minhafonteum
            \end{figure}

            \par Para verificar a aplicação rodando com a rota padrão criada pelo framework, bastou acessar a URL \textbf{http://localhost:8000/}, conforme mostra a Figura \ref{fig:figura_tutorial_7}.

            \begin{figure}[H]
                \centering
                \includegraphics[width=0.8\textwidth]{figuras/tutorial/7docker.png}
                \caption{Aplicação Laravel em execução}
                \label{fig:figura_tutorial_7}
                \newcommand{\minhafonteum}{Fonte: Autor}
                \minhafonteum
            \end{figure}
        ...
        \subsection{Codificação da API REST}
            ...
            \par Esta etapa tem como objetivo descrever o processo de codificação da API REST seguindo o tutorial do PET, levando em consideração que o projeto com o framework Laravel e o ambiente de desenvolvimento com Docker já estão organizados e rodando.
            ...
            \par O arquivo do Model ficou da seguinte forma:
            
            \lstinputlisting[language=PHP, inputencoding=utf8]{codigos/api-pet/Contact.php}
            
            \par O arquivo de Migration ficou assim:
            
            \lstinputlisting[language=PHP, inputencoding=utf8]{codigos/api-pet/Migration.php}
            
            \par Em seguida, foi necessário alterar o arquivo \texttt{bootstrap/app.php} para registrar o arquivo de rotas da API, que passou a ter a seguinte configuração:
            
            \lstinputlisting[language=PHP, inputencoding=utf8]{codigos/api-pet/app.php}
            
            \par Após o registro no \texttt{app.php}, foi criado o arquivo de rotas \texttt{routes/api.php} e definidas as rotas da API. Após a implementação, o arquivo ficou da seguinte forma:
            
            \lstinputlisting[language=PHP, inputencoding=utf8]{codigos/api-pet/api.php}

            \sloppy
            \par O último arquivo a ser criado foi o \texttt{ContactController.php}, localizado em \texttt{app/Http/Controllers}. O conteúdo do arquivo ficou da seguinte forma:
            \vspace{1cm}  % pula 1 cm

            \lstinputlisting[language=PHP, inputencoding=utf8]{codigos/api-pet/ContactController.php}
